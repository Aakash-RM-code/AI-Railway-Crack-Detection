...
class GSMCard:
    ...
    def _build_sms_form(self) -> ft.Container:
        ...
        test_btn = ft.OutlinedButton(
            ...
            style=ft.ButtonStyle(
                side=ft.border.BorderSide(1, ft.Colors.GREY_600),
                color=ft.Colors.GREY_400,
                shape=ft.RoundedRectangleBorder(radius=10),
            ),
        )
    ...
    def build(self, compact: bool = False) -> ft.Container:
        ...
        return ft.Container(
            ...
            # On narrow screens the SMS inputs can wrap to a second line, so the
            # card grows to fit instead of clipping.
            height=None if compact else 215,
        )
