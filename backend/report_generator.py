...
Standalone — depends only on the controller (runtime session data) and on the
CSV/`detections/` folder on disk. No legacy UI modules.
"""
import warnings
...
def generate_report(controller) -> str:
    """Generate a PDF inspection report for the given controller.
    ...
    gps = controller.get_gps()
    session_start = controller.started_at()
